...
```
  ...
  -f, --filename=[]: Filename or URL to file to use to create the new replication controller.
      ...
      --no-distinct-image[=false]: If true, allow the same existing image to be rolled out again. Useful for rolling out images with "latest" tag. This will make rollback impossible.
